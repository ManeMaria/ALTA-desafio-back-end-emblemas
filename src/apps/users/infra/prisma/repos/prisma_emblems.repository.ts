...
import { IEmblemsRepository, EmblemsFilters } from '@/users/application';
...
export class PrismaEmblemsRepository implements IEmblemsRepository {
  ...
  async getById(id: string): Promise<Emblems | null> {
    console.log('🚀 ~ PrismaEmblemsRepository ~ getById ~ id:', id);
    const emblemsFound = await this.prisma.emblems.findUnique({
      where: { refId: id },
    });
    console.log(
      '🚀 ~ PrismaEmblemsRepository ~ getById ~ emblemsFound:',
      emblemsFound,
    );
